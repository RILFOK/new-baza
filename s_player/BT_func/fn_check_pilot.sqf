				...
				switch (true) do {
					case ((_avia_t_1 findIf {(toLower (_x # 0)) == (toLower _className)}) >= 0): {
						if !(typeOf _player in ["O_helipilot_F","O_T_Medic_F","I_medic_F","O_T_Pilot_F"]) then {
							...
							[_player,_vehicle,1] remoteExec ["BT_fnc_time_in_sky",2];
							[_vehicle] remoteExec ["BT_fnc_enablejetdynsim",2];
						};
					};
					case ((_avia_t_2 findIf {(toLower (_x # 0)) == (toLower _className)}) >= 0): {
						if !(typeOf _player in ["O_helipilot_F","O_T_Medic_F","I_medic_F","O_T_Pilot_F"]) then {
							...
							_player setVariable ["INSKY",true];
							[_player,_vehicle,2] remoteExec ["BT_fnc_time_in_sky",2];
							[_vehicle] remoteExec ["BT_fnc_enablejetdynsim",2];
						};
					};
					case ((_avia_b_1 findIf {(toLower (_x # 0)) == (toLower _className)}) >= 0): {
						if !(typeOf _player in ["O_helipilot_F","O_T_Medic_F","I_medic_F","O_T_Pilot_F"]) then {
							...
							_player setVariable ["INSKY",true];
							[_player,_vehicle,3] remoteExec ["BT_fnc_time_in_sky",2];
							[_vehicle] remoteExec ["BT_fnc_enablejetdynsim",2];
						};
					};
					case ((_avia_b_2 findIf {(toLower (_x # 0)) == (toLower _className)}) >= 0): {
						if !(typeOf _player in ["O_helipilot_F","O_T_Medic_F","I_medic_F","O_T_Pilot_F"]) then {
							...
							_player setVariable ["INSKY",true];
							[_player,_vehicle,4] remoteExec ["BT_fnc_time_in_sky",2];
							[_vehicle] remoteExec ["BT_fnc_enablejetdynsim",2];

							if (missionNameSpace getvariable ["_enemy_aircycle_AA",true]) then {
								hint "Вражеский радар еще включен. Будьте готовы к воздушному бою после взлета";
							};
						};
					};
					...
					case ((_avia_b_storm findIf {(toLower (_x # 0)) == (toLower _className)}) >= 0): {
						if !(typeOf _player in ["O_helipilot_F","O_T_Medic_F","I_medic_F","O_T_Pilot_F"]) then {
							"Вы не на том слоте" remoteExec ["hint",_player];
							moveOut _player;
						} else {
							_dopusk = _player getVariable ["AVIA_B_STORM",0];

							//Ислючение васпа на рхс
							//if (_dopusk == 	0) exitWith {
							//	if (worldName in _allowed_world_wasp) then{
							//		"У Вас нет  допуска" remoteExec ["hint",_player];
							//		moveOut _player;
							//	} else {
							//		_dopusk2 = _player getVariable ["AVIA_B_P",0];
							//		if (_dopusk2 == 0) exitWith {
							//			"У Вас нет  допуска" remoteExec ["hint",_player];
							//			moveOut _player;
							//		};
							//	};
							...
							_player setVariable ["INSKY",true];
							[_player,_vehicle,5] remoteExec ["BT_fnc_time_in_sky",2];
							[_vehicle] remoteExec ["BT_fnc_enablejetdynsim",2];
...
// Проверка допусков на дроны. Haron

addMissionEventHandler ["PlayerViewChanged", { 
    params ["_oldUnit", "_newUnit", "_vehicle","_oldCamera", "_newCamera", "_uav"]; 

    if (!(unitIsUAV _newCamera)) exitWith{};
    if (!(_newCamera isKindOf "plane")) exitWith{};

    private _dopusk = player getVariable ["AVIA_B_P",0];
    if !(typeOf player in ["O_helipilot_F","O_T_Medic_F","I_medic_F","O_T_Pilot_F"]) exitWith{
		player connectTerminalToUAV objnull;
		player switchCamera "internal";
        hint "Вы не на том слоте";
    };

    if (_dopusk == 0) then {
		player connectTerminalToUAV objnull;
		player switchCamera "internal";
        hint "У Вас нет  допуска";
    } else {
