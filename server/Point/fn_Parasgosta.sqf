...
params ["_marker","_distance","_direction","_height","_usmc_infantry","_drop_dist"]; 

private _type = selectRandom gosta; //class name вертолета
...
if(HardReinfor == 1) then {
	[_type,_dirveh,_spawnpos,_height,_markpos,_paradist,_usmc_infantry,_drop_dist] spawn srv_fnc_desantgosta1;
} else {
	[_type,_dirveh,_spawnpos,_height,_markpos,_paradist,_usmc_infantry,_drop_dist] spawn srv_fnc_desantgosta1;
};
