...
if(_next) then {
	...
	[
		...
		true, 
		false
	] remoteExec ["BIS_fnc_holdActionAdd", independent, true];
	};
	...
	// добавить взлом перчика красной стороне. не забыть отлючить jip
	[_radioBashnya] remoteExec ["srv_fnc_holdactionbash",east,true];
    ...
    //--------------------------------укреп от БРАТА--------------------
	_radioBashnya call compile preprocessfilelinenumbers "serverconfig\broturrets\bash3_placing.sqf";
